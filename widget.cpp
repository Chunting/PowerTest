    ...
    ui(new Ui::Widget)
{
    ...
    sysWidget=new SystemWidget(ui->chartPage);
    qreal widget_width=sysWidget->width();
    qreal widget_height=sysWidget->height();
...
void Widget::on_sellOkBtn_clicked()
{
    ...
    QMessageBox::information(this,tr("提示"),tr("系统参数设定成功！！"),QMessageBox::Ok);
    writeXml();
    showDailyList();
    on_sellCancelBtn_clicked();
    //    }
...
void Widget::on_nextRunBtn_clicked()
{
    on_chartMenu_clicked();
...
void Widget::createNodes(QDomElement &date)
{
    ...
    text = doc.createTextNode(QString("%1").arg(ui->sellPriceLineEdit->toPlainText()));
...
void Widget::createChartModelView()
{
    ...
    qreal _swidth=ui->stackedWidget->width();
    qreal _sheight=ui->stackedWidget->height();
    splitter->resize(_swidth-30,_sheight-60);
    splitter->move(0,50);
    splitter->addWidget(sysWidget);
    //    splitter->resize(800,560);
...
{
    //    if(ui->factoryComboBox->currentText()!="请选择系统")
    //        showChart();
}
/*
...
{
    // ui->stackedWidget->setCurrentIndex(sysWidIndex);
    ui->stackedWidget->setCurrentIndex(1);
}
...
{
    manageAction = new QAction(tr("系统参数"), this);
    chartAction = new QAction(tr("运行"), this);
    ...
    manageMenu = menuBar()->addMenu(tr("系统选择"));
